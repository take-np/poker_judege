...
describe HomeController do
  ...
  four=[1,1,2,1,1]
  it "フォーオブアカインド" do
    expect(HomeController.new.nonStraight(four)).to eq "フォーオブアカインド"
  end
  house = [3, 13, 3, 13, 13]
  it "フルハウス" do
    expect(HomeController.new.nonStraight(house)).to eq "フルハウス"
  end
  three = [13, 13, 13, 12, 11]
  it "スリー・オブ・ア・カインド" do
    expect(HomeController.new.nonStraight(three)).to eq "スリー・オブ・ア・カインド"
  end
  two = [13, 13, 11, 10, 11]
  it "ツーペア" do
    expect(HomeController.new.nonStraight(two)).to eq "ツーペア"
  end
  one = [13, 13, 10, 9, 8]
  it "ワンペア" do
    expect(HomeController.new.nonStraight(one)).to eq "ワンペア"
  end
  no = [1, 4, 5, 13, 10]
  it "役なし" do
    expect(HomeController.new.nonStraight(no)).to eq "役なし"
  end


 #  @numbers=[1, 2, 3, 4, 5]
 # it "足し算でできること" do
 #   expect(HomeController.new.judgeStraightFlash(@numbers)).to eq true
 # end
end
